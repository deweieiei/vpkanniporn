-- ============================================================
--  ⛔⛔ ห้ามรันไฟล์นี้ — เก็บไว้เป็นประวัติเท่านั้น ⛔⛔
--  (ติดป้ายเมื่อ 2026-07-16 หลังรันจริงแล้วพังซ้ำๆ)
--
--  ไฟล์นี้พังบน production 2 จุด:
--    1. ส่วนตรวจสอบใช้ SELECT จาก information_schema
--       → HostAtom ไม่ให้ DB user อ่าน → #1044 Access denied → หยุดกลางคัน
--    2. ใช้ CREATE TABLE IF NOT EXISTS ทั้งที่ตารางถูกสร้างไปแล้วด้วยเวอร์ชันเก่า
--       (user_id NOT NULL) → คำสั่งถูกข้าม → INSERT NULL ไม่ได้
--       → ERROR 1048: Column 'user_id' cannot be null
--
--  ★ ให้ใช้ไฟล์พวกนี้แทน:
--      db/insert_blocks_only.sql  = ตัวที่รันสำเร็จจริงบน production (ตารางมีอยู่แล้ว)
--      db/repair_blocks.sql       = DROP + CREATE + INSERT ครบชุด (ถ้าต้องเริ่มใหม่)
--      db/diagnose_blocks.sql     = ตรวจอาการอย่างเดียว ไม่แก้อะไร
--
--  ดูรายละเอียดเต็มที่ AI_Memory/HANDOFF_2026-07-16.txt
-- ============================================================
