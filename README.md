...
## Support Admin — จัดการเนื้อหาหน้าแรก (index)

ระบบให้ "Support Admin" ล็อกอินแยกเพื่อแก้ **ข้อความและรูป** บนหน้าแรกได้เอง โดยไม่ต้องแตะโค้ด
(เนื้อหาเก็บในตาราง `site_content` แบบ key→ค่า; หน้า index ดึงมาแสดง ถ้าดึงไม่ได้ใช้ข้อความ static เป็น fallback)

**ติดตั้งครั้งแรก**
1. รัน `db/migration_support_admin.sql` (phpMyAdmin → tab SQL → paste → Go)
   — เพิ่ม role `support_admin`, สร้างตาราง `site_content` + ใส่ค่าตั้งต้นจากหน้า index ปัจจุบัน
2. สร้างบัญชี Support Admin:
   ```
   node scripts/create_support_admin.js <email> <password> [ชื่อ] [นามสกุล]
   ```
   (รันซ้ำเพื่อรีเซ็ตรหัสผ่านได้)

**การใช้งาน**
- เข้าหน้า login แยกที่ `/support-admin` → เข้าสู่ระบบ → ไปหน้าแก้เนื้อหา `/support-admin/editor`
...
| GET | `/api/content/admin` | support admin | ข้อมูลเต็ม (section/label/type) ไว้สร้างฟอร์ม editor |
| PUT | `/api/content` | support admin | บันทึกข้อความหลายช่อง (`{updates:{key:value}}`) |
| POST | `/api/content/image` | support admin | อัปโหลดรูปแทน 1 key (multipart: `key` + `image`) |
